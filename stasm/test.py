...
# ]IMPORTANT] THIS SCRIPT IS JUST AN MINIMAL MODIFICATION OF ANOTHER SCRIPT, SEE README.md]]
...
def run_test_for_file(file_path: str, stats: RunStats = RunStats()):
    ...
    assert file_path.endswith(STASM_EXT)

    print('[INFO] Testing %s' % file_path)

    tc_path = file_path[:-len(STASM_EXT)] + ".txt"
    tc = load_test_case(tc_path)

    error = False

    if tc is not None:
        com = cmd_run_echoed([stasm, "-r", "../bin/stm", "-f" , file_path, "-o", "tmp.stm", "-s", *tc.argv], input=tc.stdin, capture_output=True)
        ...
        com = cmd_run_echoed([stasm, "-r", "../bin/stm", "-f", file_path, "-o", "tmp.stm", "-s"])
        if com.returncode != 0:
            error = True
            stats.failed += 1
        stats.ignored += 1
...
def run_test_for_folder(folder: str):
    ...
    for entry in os.scandir(folder):
        if entry.is_file() and entry.path.endswith(STASM_EXT):
...
def update_output_for_file(file_path: str):
    ...
    output = cmd_run_echoed([stasm, "-r", "../bin/stm", "-f", file_path, "-o", "tmp.stm", "-s", *tc.argv], input=tc.stdin, capture_output=True)
...
def usage(exe_name: str):
    ...
    print("  SUBCOMMAND:")
    print("    run [TARGET]")
    print("      Run the test on the [TARGET]. The [TARGET] is either a *.stasm file or ")
    print("      folder with *.stasm files. The default [TARGET] is './examples/'.")
    print()
    ...
    print()
    print("        output [TARGET]")
    print("          Update the output of the [TARGET]. The [TARGET] is either a *.stasm")
    print("          file or folder with *.stasm files. The default [TARGET] is")
    print("          './examples/'")
    print()
